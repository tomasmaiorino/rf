...
public class TransferParser {
    ...
    public Transfer toModel(final TransferResource resource) {
        ...
        return Transfer.TransferBuilder.Transfer(resource.getDestinationAccount(), resource.getOriginAccount(),
            LocalDate.parse(resource.getScheduleDate(), formatter).atStartOfDay(), resource.getTransferValue()).build();
    }
